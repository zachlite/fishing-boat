...
const plane = primitivePlane(10, 10, 200, 200);
  ...
  const drawOcean = regl({
    vert: `
      ...
      float computeWaveHeight(vec3 pos, float time, WaveParams wave) {
        ...
        float waveHeight = A * sin(dot(normalize(D), pos.xy) * f + (time * Phi));

        return waveHeight;
      }
  
      void main () {
        WaveParams w1 = WaveParams(0.1, 1.0, .5, vec2(1.0, 0.0));
        WaveParams w2 = WaveParams(0.02, .40, .5, vec2(.7, .2));
        WaveParams w3 = WaveParams(0.0435, .75, .4, vec2(1.243, .83));
        WaveParams w4 = WaveParams(0.6, 2.83, .2, vec2(.13, 1.0283));
        WaveParams w5 = WaveParams(0.334, .75, .4, vec2(.62, 1.233));
    ...
    frag: `
      ...
      void main () {
        ...
        vec3 ambient = vec3(.3) * oceanBaseColor.xyz;
